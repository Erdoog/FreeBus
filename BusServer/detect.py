...
bus_ref = ref.child(CURRENT_BUS_ID)
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if (names[c] == 'person' and conf > 0.5):
        ...
        if (current_person_count != person_count):
            person_count = current_person_count
            bus_ref.update({
                'route': BUS_ROUTE_ID,
                'person_count': f"{current_person_count}",
                'capacity': BUS_CAPACITY,
                'location': location,
            })
